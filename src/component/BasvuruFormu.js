...
function BasvuruFormu(){
    ...
    const handleSubmit = async (e) => {
        if (ad, soyad, dogumTarihi, eMail, telNo,cinsiyet, milliyet1, tcPasapportNo, engelDurum, EngelDurum_aciklama, mezuniyetdurum,mezuniyetTarih, ortalama,universite, fakulte,bolum,ulke,il,ilce,postaKutusu,mahalle,adr_aciklama ,cvbelge, niyetbelge, pasaportbelge, ikametgahbelge, diplomabelge, ingyeterlilikbelge != '') {
            e.preventDefault();
            try {
                const response = await axios.post("http://localhost:3001/formGonder",{id,ad, soyad, dogumTarihi, eMail, telNo,cinsiyet, milliyet1, tcPasapportNo, engelDurum, EngelDurum_aciklama, mezuniyetdurum,mezuniyetTarih, ortalama,universite, fakulte,bolum,ulke,il,ilce,postaKutusu,mahalle,adr_aciklama ,cvbelge, niyetbelge, pasaportbelge, ikametgahbelge, diplomabelge, ingyeterlilikbelge}
                );
                console.log(response);
                if (response.status === 200) {
                    setSuccess('Başvurunuz oluşturuldu. ');      
                    setError('');
                    setAd('');
                    setSoyad('');        
                    setDogumTarihi('');
